...
describe User do
	...
	it "should not allow a non-email address" do
		client = create(:client, users_attributes: [attributes_for(:user)])
		build(:user, email: "matt", client_id: client.id).should_not be_valid
	end
	...
	context "client user email already exists" do
		before do
			...
			@other_client = create(:client, is_account_master: false, name: same_client_name, users_attributes: [attributes_for(:user)])
			...
			me_vendor = vendor.users.first
			@my_client = create(:client, name: same_client_name, account_id: me_vendor.client.account.id, users_attributes: [attributes_for(:user)])
		end
		...
		it "should not allow another client with the same email address to be created in the same account" do
			other_client_2 = create(:client, account_id: @other_client.account.id, users_attributes: [attributes_for(:user)])
		...
		it "should allow a vendor with the same email address to be created" do
			...
			new_vendor = create(:client, is_account_master: true, account_id: new_account.id, users_attributes: [attributes_for(:user)])
	...
	context "vendor user email already exists" do
		before do
			...
			other_vendor = create(:client, is_account_master: true, users_attributes: [attributes_for(:user)])
		...
		it "should allow a client with the same email to be created under another account" do
			...
			build(:user, email: @taken_vendor_email, client_id: new_client.id).should be_valid
		end
	end
